...
if st.button("CRITIQUE!", type="primary", disabled=not st.session_state.upload):
    ...
    with st.spinner("Critiquing..."):
        ...
        if st.session_state.upload:
            image_bytes = st.session_state.upload.getvalue()
            image_ext = Path(st.session_state.upload.name).suffix
            image_filename = f"{ts_str}_{selected_critic_name.lower().replace(' ', '_')}{image_ext}"
            image_path = IMAGE_LOG_DIR / image_filename
            with open(image_path, "wb") as f:
                f.write(image_bytes)

        # The audio is generated and saved later, so we prepare the path
        audio_filename = f"critique_{selected_critic_name.lower().replace(' ', '_')}_{ts_str}.mp3"
        audio_path = AUDIO_LOG_DIR / audio_filename
        ...
        db.insert({
            ...
            "audio_path": str(audio_path) # We'll save the file to this path later
        })
...
if st.session_state.critique_data:
    ...
    # This runs after audio is generated by ElevenLabs or OpenAI
    if st.session_state.audio_to_download:
        # Save the generated audio file if a path is set
        if 'current_audio_path' in st.session_state and st.session_state.current_audio_path:
            with open(st.session_state.current_audio_path, "wb") as f:
                f.write(st.session_state.audio_to_download)
            # Clear the path after saving to avoid re-saving
        ...
        st.download_button(
            ...
            file_name=Path(st.session_state.current_audio_path).name if 'current_audio_path' in st.session_state and st.session_state.current_audio_path else f"critique_{selected_critic_name.lower().replace(' ', '_')}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.mp3",
            mime="audio/mpeg"
        )
